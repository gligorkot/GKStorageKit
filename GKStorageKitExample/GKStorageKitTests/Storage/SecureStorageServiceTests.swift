...
class SecureStorageServiceTests: XCTestCase, StorageKitDecorator {
    ...
    func test_storageServiceErrorStoreString() {
        ...
        waitForExpectations(timeout: defaultTimeout) { (_) in
            if let error = error {
                expect(error.title) == "Storage"
                expect(error.message) == "Cannot save secure value. Keychain inaccessible."
            } else {
                XCTFail()
            }
        }
    ...
    func test_storageServiceKeychainInaccessibleErrorGetString() {
        ...
        waitForExpectations(timeout: defaultTimeout) { (_) in
            if let error = error {
                expect(extractedValue).to(beNil())
                expect(error.title) == "Storage"
                expect(error.message) == "Cannot retrieve secure value. Keychain inaccessible."
            } else {
                XCTFail()
            }
        }
    ...
    func test_storageServiceErrorRemoveValue() {
        ...
        waitForExpectations(timeout: defaultTimeout) { (_) in
            if let error = error {
                expect(error.title) == "Storage"
                expect(error.message) == "Cannot remove secure value. Keychain inaccessible."
            } else {
                XCTFail()
            }
        }
    ...
    func test_storageServiceErrorCleanSecureStorage() {
        ...
        waitForExpectations(timeout: defaultTimeout) { (_) in
            if let error = error {
                expect(error.title) == "Storage"
                expect(error.message) == "Cannot clean secure storage. Keychain inaccessible."
            } else {
                XCTFail()
            }
        }
